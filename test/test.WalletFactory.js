...
console.log('[test.WalletFactory.js.166:original:]',k,original[k]); //TODO
  ...
  describe('#joinCreateSession', function() {
    ...
    it('should yield bad network error', function(done) {
      wf.network.greet = sinon.stub();
      wf.network.on = sinon.stub();
      wf.network.on.withArgs('data').yields('senderId', {
        type: 'walletId',
    ...
    it('should yield to join error', function(done) {
      ...
      wf.network.greet = sinon.stub();
      wf.network.on = sinon.stub();
      wf.network.on.withArgs('serverError').yields(null);
      wf.network.on.withArgs('data').yields('senderId', {
        type: 'walletId',
    ...
    it('should call network.start / create', function(done) {
      ...
      wf.network.on = sinon.stub();
      wf.network.on.withArgs('connected').yields(null);
      wf.network.on.withArgs('data').yields('senderId', {
        type: 'walletId',
        networkName: wf.networkName,
        opts: {},
