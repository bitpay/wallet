...
describe('WalletFactory model', function() {
  ...
  beforeEach(function() {
    ...
    wf.network.setHexNonce = sinon.spy();
  });
  ...
  describe('#joinCreateSession', function() {
    ...
    it('should yield network error', function(done) {
      opts.privHex = undefined;
      wf.network.on = sinon.stub().withArgs('serverError').yields(null);
    ...
    it('should call network.start', function(done) {
      opts.privHex = undefined;
      wf.network.cleanUp = sinon.spy();
      wf.network.greet = sinon.spy();
      wf.network.on = sinon.stub().withArgs('connected').yields(null);

      wf.network.start = sinon.stub().yields(null);
      wf.joinCreateSession(opts, function(err, w) {
        console.log('[test.WalletFactory.js.454:err:]', err); //TODO
