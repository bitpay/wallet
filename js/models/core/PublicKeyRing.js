...
PublicKeyRing.fromObj = function(data) {
  assert(!(data instanceof PublicKeyRing), 'bad data format: Did you use .toObj()?');
 ...
 * @return {string} the newEpk parameter
 */
PublicKeyRing.prototype.addCopayer = function(newHexaExtendedPublicKey, nickname) {
  assert(newEpk && _.isString(newEpk));
  assert(!this.isComplete());
  assert(!nickname || _.isString(nickname));
  assert(!_.any(this.copayersHK,
...
PublicKeyRing.prototype.getRedeemScriptMap = function(paths) {
  ...
  for (var i = 0; i < paths.length; i++) {
    var path = paths[i];
    var script = this.getRedeemScript(path.addressIndex, path.isChange, path.copayerIndex);
