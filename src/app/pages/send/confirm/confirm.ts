...
export class ConfirmPage {
  ...
  constructor(
    ...
    protected events: EventManagerService,
    ...
    this.isDonation
      ? (this.receiveLotusAddress = this.navParamsData.receiveLotusAddress)
      : (this.receiveLotusAddress = null);
    this.nameReceiveLotusAddress = this.navParamsData.nameReceiveLotusAddress;
    this.nameReceiveLotusAddress
      ? (this.isShowReceive = true)
      : (this.isShowReceive = false);
    this.fromWalletDetails = this.navParamsData.fromWalletDetails;
    this.walletConnectRequestId = this.navParamsData.walletConnectRequestId;
  ...
  loadInit() {
    ...
    if (this.fromMultiSend) {
      ...
      amount = this.navParamsData.totalAmount;
    ...
    this.tx = {
      ...
      paypro: this.navParamsData.paypro,
      data: this.navParamsData.data, // eth
      ...
      email: this.navParamsData.email,
      color: this.navParamsData.color,
      network: this.navParamsData.network
        ? this.navParamsData.network
        : networkName,
      coin: this.navParamsData.coin,
      txp: {},
      multisigContractAddress: this.navParamsData.multisigContractAddress,
      tokenAddress: this.navParamsData.tokenAddress,
      gasLimit: this.navParamsData.gasLimit,
      speedUpTx: this.isSpeedUpTx,
      fromSelectInputs: this.navParamsData.fromSelectInputs ? true : false,
      inputs: this.navParamsData.inputs,
      nonce: this.navParamsData.nonce
    };

    this.tx.sendMax = this.navParamsData.useSendMax ? true : false;

    this.tx.amount =
      this.navParamsData.useSendMax && this.shouldUseSendMax()
        ? 0
        : this.tx.coin == 'eth' ||
    ...
    if (this.navParamsData.requiredFeeRate) {
      ...
      this.tx.feeLevel =
        this.navParamsData.coin == 'eth' ? 'priority' : 'custom';
    } else {
  ...
  formatAmount(coin, satoshisAmout) {
    return this.txFormatProvider.formatAmount(coin, satoshisAmout);
  }
  ...
  private setCoinbaseAccount(option): void {
    ...
    if (this.tx.paypro) {
      ...
      this.tx.minerFee = this.navParamsData.minerFee;
  ...
  private setButtonText(
    ...
    if (isPayPro) {
      ...
      if (
        ...
        (this.navParamsData.isEthMultisigConfirm ||
          this.navParamsData.isEthMultisigExecute)
      ) {
  ...
  getFeeRate(amount: number, fee: number) {
    return (fee / (amount + fee)) * 100;
  }

  isHighFee(amount: number, fee: number) {
  ...
  private getSendMaxInfo(tx, wallet): Promise<any> {
    return new Promise((resolve, reject) => {
      if (!tx.sendMax) return resolve(undefined);
  ...
  private getSpeedUpTxInfo(tx, wallet): Promise<any> {
    ...
    return this.walletProvider
      .getTx(wallet, this.navParamsData.txid)
      .then(res => {
        this.onGoingProcessProvider.clear();
  ...
  private getTxp(tx, wallet, dryRun: boolean): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      if (this.fromMultiSend) {
        txp.outputs = [];
        this.navParamsData.recipients.forEach(recipient => {
      ...
      if (
        ...
        !this.navParamsData.isEthMultisigInstantiation &&
        !this.navParamsData.isEthMultisigConfirm &&
        !this.navParamsData.isEthMultisigExecute
      ) {
          ...
          if (!output.data) {
            output.data = this.bwcProvider
              ...
              .confirmTransactionEncodeData({
                multisigContractAddress: tx.multisigContractAddress,
                transactionId: +this.navParamsData.transactionId
              });
          }
        }
      }
      ...
      if (this.isDonation && this.navParamsData.receiveLotusAddress) {
        txp.isDonation = true;
        txp.receiveLotusAddress = this.navParamsData.receiveLotusAddress;
      }
    ...
    setTimeout(async () => {
      ...
      if (multisigContractInstantiationInfo.length > 0) {
        ...
        const multisigEthInfo = {
          ...
          walletName: this.navParamsData.walletName,
          n: this.navParamsData.totalCopayers,
          m: this.navParamsData.requiredConfirmations
        };
  ...
  private getInput(wallet): Promise<any> {
    return this.walletProvider.getUtxos(wallet).then(utxos => {
      ...
      let input;
      _.forEach(utxos, (u, i) => {
        if (u.txid === this.navParamsData.txid) {
  ...
  public async chooseFeeLevel(): Promise<void> {
    ...
    const chooseFeeLevelModal = await this.modalCtrl.create({
      component: ChooseFeeLevelModal,
      componentProps: txObject
    });
    await chooseFeeLevelModal.present();
    chooseFeeLevelModal.onDidDismiss().then(({ data }) => {
  ...
  private handleError(err, exit?) {
    ...
    if (isInsufficientFundsErr) {
      ...
      } else if (this.navParamsData.isFromAmountPage === 'AmountPage') {
  ...
  public openScanner(): void {
    this.router.navigate(['/scan'], { state: { fromConfirm: true } });
  }
