...
export class RevealAtScrollPosition {
  ...
  ngAfterViewInit() {
    ...
    this.scrollArea.ionScroll.subscribe(
      (event : any) =>
        this.shouldUpdateStyling(event.detail.scrollTop) &&
        this.updateStyling(event.detail.scrollTop)
    );
  }
  ...
  setInitialStyles() {
    ...
    this.renderer.addClass(this.element.nativeElement, 'ion-text-center');
  }
