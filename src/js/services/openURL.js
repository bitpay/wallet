  ...
  root.init = function() {
    ...
    if (platformInfo.isChromeApp) {
      ...
      gui.App.on('open', function(pathData) {
        if (pathData.indexOf(/^terracoin:/) != -1) {
          $log.debug('Terracoin URL found');
          handleOpenURL({
            url: pathData.substring(pathData.indexOf(/^terracoin:/))
          });
      ...
      if (navigator.registerProtocolHandler) {
        ...
        navigator.registerProtocolHandler('terracoin', url, 'TRCpay Terracoin Handler');
        navigator.registerProtocolHandler('web+terracoin', url, 'Terrcoin Wallet Handler');
        navigator.registerProtocolHandler('web+trcpay', url, 'TRCpay Wallet Handler');
      }
    }
  };
