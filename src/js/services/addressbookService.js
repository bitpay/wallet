...
angular.module('copayApp.services').factory('addressbookService', function($log, bitcore, bitcoreCash, storageService, lodash) {
    ...
    root.list = function(cb) {
        ...
        var last = networks.length;

        var result = {};
        var callBackFunc = function(index, last, networks) {
            storageService.getAddressbook(networks[index].name, function(err, ab) {
                if (err) return cb('Could not get the Addressbook');
                if (ab) ab = JSON.parse(ab);
                ab = ab || {};
                result = lodash.defaults(ab, result);
                if (index == (last - 1)) {
                    cb(err, result);
                } else {
                    callBackFunc(index + 1, last, networks);
                }
            });
        };
        callBackFunc(0, last, networks);
        /*
        storageService.getAddressbook('testnet', function(err, ab) {
            if (err) return cb('Could not get the Addressbook');
    ...
    root.add = function(entry, cb) {
        ...
        if (/[^:]*:/.test(entry.address)) {
            var url = new URL(entry.address);
            entry.address = url.pathname;
