...
angular.module('copayApp.controllers').controller('backupRequestController', function($scope, $state, $stateParams, $ionicConfig, popupService, gettextCatalog) {
  ...
  $scope.openPopup = function() {
    ...
    var confirmText = gettextCatalog.getString('I understand');
    var cancelText = gettextCatalog.getString('Go back');
    var actionIsDiscouraged = true;
    popupService.showConfirm(title, message, confirmText, cancelText, function(val) {
      if (val) {
        ...
        var confirmText = gettextCatalog.getString('Yes, skip backup');
        var cancelText = gettextCatalog.getString('Go back');
        popupService.showConfirm(title, message, confirmText, cancelText, function(val) {
          if (val) {
