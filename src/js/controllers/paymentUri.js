...
angular.module('copayApp.controllers').controller('paymentUriController',
  function($rootScope, $scope, $stateParams, $location, $timeout, $ionicHistory, profileService, configService, lodash, bitcore, $state) {
    ...
    // Build terracoinURI with querystring
    this.init = function() {
      var query = [];
      this.terracoinURI = $stateParams.url;

      var URI = bitcore.URI;
      var isUriValid = URI.isValid(this.terracoinURI);
      if (!URI.isValid(this.terracoinURI)) {
        this.error = true;
        return;
      }
      var uri = new URI(this.terracoinURI);
    ...
    this.selectWallet = function(wid) {
      ...
      $state.go('tabs.home');
      $timeout(function() {
        $rootScope.$emit('paymentUri', self.terracoinURI);
      }, 1000);
    };
  });
