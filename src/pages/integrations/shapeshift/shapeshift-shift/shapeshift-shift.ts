...
export class ShapeshiftShiftPage {
  ...
  constructor(
    ...
    this.fromWallets = _.filter(this.walletsBtc.concat(this.walletsBch), (w: any) => {
      ...
      const hasCachedFunds = w.cachedBalance.match(/0\.00 /gi) ? false : true;
  ...
  public openTerms() {
    const url = "https://info.shapeshift.io/sites/default/files/ShapeShift_Terms_Conditions%20v1.1.pdf";
  ...
  private showToWallets(): void {
    ...
    const pair = this.fromWallet.coin + '_' + this.toWallet.coin;
    this.shapeshiftProvider.getRate(pair, (err: any, rate: number) => {
      ...
      this.shapeshiftProvider.getMarketInfo(pair, (err: any, limit: any) => {
        ...
        if (this.limit.rate == 0 || this.rate.rate == 0) {
          const msg = this.translate.instant('ShapeShift is not available at this moment. Please, try again later.');
  ...
  public showWallets(selector: string): void {
    ...
    const title: string = selector == 'from' ? this.fromWalletSelectorTitle : this.toWalletSelectorTitle
    if (selector == 'from') {
