...
export class SellGlideraPage {
  ...
  private ask2FaCode(mode, cb): Function {
    if (mode != 'NONE') {
      // SHOW PROMPT
      const title = 'Please, enter the code below';
      let message;
      if (mode == 'PIN') {
  ...
  public sellConfirm(): void {
    const message = 'Sell bitcoin for ' + this.amount + ' ' + this.currency;
        ...
        this.ask2FaCode(tfa.mode, (twoFaCode) => {
          ...
          this.walletProvider.getAddress(this.wallet, false).then((refundAddress) => {
            ...
            this.glideraProvider.getSellAddress(this.token, (err, sellAddress) => {
              ...
              const amount = parseInt((this.sellInfo.qty * 100000000).toFixed(0));
              const comment = 'Glidera transaction';
  ...
  public openSuccessModal(): void {
    const successText = 'Funds sent to Glidera Account';
    const successComment = 'The transaction is not yet confirmed, and will show as "Pending" in your Activity. The bitcoin sale will be completed automatically once it is confirmed by Glidera';
