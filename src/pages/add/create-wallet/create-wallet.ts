...
export class CreateWalletPage implements OnInit {
  ...
  public setDerivationPath(): void {
    const path: string = this.createForm.value.testnet ? this.derivationPathForTestnet : this.derivationPathByDefault;
  ...
  public setOptsAndCreate(): void {
    ...
    if (setSeed) {
      ...
      const pathData = this.derivationPathHelperProvider.parse(this.createForm.value.derivationPath);
      if (!pathData) {
