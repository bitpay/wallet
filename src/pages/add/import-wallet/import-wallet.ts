...
export class ImportWalletPage {
  ...
  private processWalletInfo(code: string): void {
    ...
    if (parsedCode.length != 5) {
      ...
      const subtitle = this.translate.instant('Incorrect code format');
  ...
  public setDerivationPath(): void {
    const path = this.testnetEnabled ? this.derivationPathForTestnet : this.derivationPathByDefault;
  ...
  public importFromMnemonic(): void {
    ...
    const pathData: any = this.derivationPathHelperProvider.parse(this.importForm.value.derivationPath);
    ...
    if (!words) {
      ...
      const wordList: any[] = words.split(/[\u3000\s]+/);
