...
export class IncomingDataProvider {
  ...
  public redir(data: string): boolean {
    ...
    if (this.bwcProvider.getBitcore().URI.isValid(data)) {
      ...
      const title = this.translate.instant('Bitcoin cash Payment');
      ...
      if (this.navCtrl.getActive().name === 'ScanPage') {
        ...
        const coin = 'btc';
        const network = this.addressProvider.validateAddress(data).network;
  ...
  private handlePayPro(payProDetails: any, coin?: string): void {
    const stateParams = {
      ...
      coin,
      network: this.addressProvider.validateAddress(payProDetails.toAddress)
        .network
    };
