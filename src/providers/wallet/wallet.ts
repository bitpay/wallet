...
export class WalletProvider {
          ...
          const getNewTxs = (newTxs: any[], skip: number): Promise<any> => {
            return new Promise((resolve, reject) => {
              this.getTxsFromServer(wallet, skip, endingTxid, requestLimit)
                ...
                .catch(err => {
                  this.logger.warn(
                    this.bwcErrorProvider.msg(err, 'Server Error')
                  ); //TODO
                  if (
                    err instanceof this.errors.CONNECTION_ERROR ||
                    (err.message && err.message.match(/5../))
  ...
  public getMinFee(wallet: any, nbOutputs?: number): Promise<any> {
    return new Promise((resolve, reject) => {
      this.feeProvider
        ...
        .then((data: any) => {
          const normalLevelRate: any = lodash.find(
  ...
  public updateRemotePreferences(clients: any, prefs?: any): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      const updateRemotePreferencesFor = (
        clients: any,
        prefs: any
      ): Promise<any> => {
        return new Promise((resolve, reject) => {
  ...
  public decrypt(wallet: any): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      this.askPassword(
        null,
        this.translate.instant('Enter Spending Password')
      ).then((password: string) => {
        if (!password) {
          return reject(this.translate.instant('no password'));
        }
