...
describe('AddressBookProvider', () => {
  ...
  describe('get function', () => {
    it('If an address is tesnet and the address exist, get function will return the address book', () => {
      ...
      const response: string = '{"mnH3XUZ8CmH8CMruEfCDXGc83XLSn8szbm":{"address":"mnH3XUZ8CmH8CMruEfCDXGc83XLSn8szbm","email": "test@test.com","name": "test"}}';
      const promise = Promise.resolve(response);
      const getAddressBookSpy = spyOn(persistenceProvider, 'getAddressBook').and.returnValue(promise);

      addressBookProvider.get(addr).then(result => {
    ...
    it('If an address does not exist on Address Book, getAddressBook function will try again with livenet', () => {
      ...
      addressBookProvider.get(addr).then(() => {
        expect(persistenceProvider.getAddressBook).toHaveBeenCalledWith('livenet')
      }).catch(err => expect(err).toBeNull);
    });
  ...
  describe('add function', () => {
    ...
    it('Add an entry to addressbook if everything is ok', () => {
      ...
      addressBookProvider.add(entry).then(() => {
        expect(persistenceProvider.getAddressBook).toHaveBeenCalledWith('testnet');
      }).catch(err => expect(err).toBeNull);
    });
  });
  ...
  describe('remove function', () => {
    ...
    it('Remove an address book that exist', () => {
      ...
      spyOn(persistenceProvider, 'getAddressBook').and.returnValue(promise);
      addressBookProvider.remove(address).then(result => {
        const expectedResult = {
          "1Q2aGBWZHNuZ7WjB9komwj9fF9GmnK5AzU": {
            address: "1Q2aGBWZHNuZ7WjB9komwj9fF9GmnK5AzU",
            email: "test@test.com",
    ...
    it('If setAddressBook fails, add function will reject', () => {
      ...
      spyOn(persistenceProvider, 'getAddressBook').and.returnValue(getAddressBookPromise);
      spyOn(persistenceProvider, 'setAddressBook').and.returnValue(setAddressBookPromise);
      addressBookProvider.remove(address).catch(err => {
        expect(err).toEqual(errorMsg);
      });
    });
