...
export class TxFormatProvider {
  ...
  public parseAmount(coin: string, amount, currency: string, onlyIntegers?: boolean) {
    let settings = this.configProvider.get().wallet.settings;
    ...
    if (currency != 'BCH' && currency != 'BTC' && currency != 'sat') {
      let formattedAmount = onlyIntegers ? Number(this.filter.formatFiatAmount(amount)).toFixed(0) : this.filter.formatFiatAmount(amount);
