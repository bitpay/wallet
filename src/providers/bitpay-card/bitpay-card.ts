...
export class BitPayCardProvider {
  ...
  private _buildDate(date, time) {
    ...
    let newDate = new Date(date[1] + '/' + date[2] + '/' + date[3]);
  ...
  private _getMerchantInfo(tx) {
    let bpTranCodesTemp = bpTranCodes;
  ...
  private _getIconName(tx) {
    let icon = tx.mcc || tx.category || null;
  ...
  public _fromTransaction(txn, runningBalance) {
    let dateTime = this._buildDate(txn.date, txn.time);
    let merchant = this._lowercaseMerchant(txn.merchant);
  ...
  public _processTransactions(invoices, history) {
    ...
    let balance = history.endingBalance || history.currentCardBalance;
    ...
    if (activityList.length > 0) {
      ...
      for (let i = 0; i < invoices.length; i++) {
        let matched = false;
        for (let j = 0; j < history.transactionList.length; j++) {
          let description = history.transactionList[j].description;
        ...
        let isInvoiceLessThanOneDayOld = moment() < moment(new Date(invoices[i].invoiceTime)).add(1, 'day');

        if (!matched && isInvoiceLessThanOneDayOld) {
          let isInvoiceUnderpaid = invoices[i].exceptionStatus === 'paidPartial';
