...
export class ReleaseProvider {
  ...
  public getLatestAppVersion(): Promise<any> {
  ...
  private verifyTagFormat(tag: string): any {
    var regex = /^v?\d+\.\d+\.\d+$/i;
    return regex.exec(tag);
  }

  private formatTagNumber(tag: string): any {
  ...
  public checkForUpdates(latestVersion: string, currentVersion?: string) {
    ...
    if (!this.verifyTagFormat(currentVersion))
      result.error = 'Cannot verify the format of version tag: ' + currentVersion;
    if (!this.verifyTagFormat(latestVersion))
      result.error = 'Cannot verify the format of latest release tag: ' + latestVersion;
